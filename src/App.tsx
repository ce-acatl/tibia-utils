...
function App() {
  ...
  return (
    <div className="App">
      ...
      {
        (links.map((link) => (<button onClick={handleClick(link)} key={link[0]}>{link[1]}</button>)))
      }
      {
        currentScreen[0] === 'chronometers' &&
        chronos.map((chrono: any, i) => {
          return (
            <Chronometer key={`chrono-${i}`} {...chrono} />
          )
        })
      }
      {
        currentScreen[0] === 'inbuiments' &&
        <Inbuiments />
      }
      {
        currentScreen[0] === 'questItems' &&
        <QuestItems />
      }
    </div>
  );
