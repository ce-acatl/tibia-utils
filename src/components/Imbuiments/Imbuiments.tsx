...
const Imbuiments = () => {
    const [inbuimentProductsFiltered, setInbuimentProductsFiltered] = React.useState(inbuimentProducts);
    ...
    const calculateInbuiments = (lvl: number) => {
        return () => {
            setCalculetedInbuiments(inbuimentProducts.reduce((acc: any, curr: any) => {
    ...
    return (
        <>
                                ...
                                inbuiments.map((i) => {
                                    return (
                                        <tr key={i}>
                                            ...
                                            <td>{c(cI[i][0]) || `??`}</td>
                                            <td>{c(cI[i][1]) || `??`}</td>
                                            <td>{c((cI[i] as number[]).reduce((a, b) => ((a || b) + (b || a)) / 2, 0))}</td>
                                        </tr>
                                    )
                                })
                            }
            ...
            <>
                ...
                <input value={filter} onChange={handleChange} type="text" placeholder="Search" />
